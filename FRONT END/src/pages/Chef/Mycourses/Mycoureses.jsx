...
import { useChefsCoursMutation,useHandleShowCourseMutation } from '../../../api/chefApiSlice';
...
function Mycoureses() {
  ...
  const handleshowcourses = async (id) => {
    try {
      ...
      if(listAllClicked){
        const trueCourses = response.data.courses.filter((course) => course.isShow === true);
        setList(trueCourses)
      }else{
        const trueCourses = response.data.courses.filter((course) => course.isShow === false);
        setList(trueCourses)
      }
  ...
  const handleListAll = async () => {
    ...
    // Handle logic for listing all courses
  ...
  const handleUnlistAll = async () => {
    ...
    // Handle logic for unlisting all courses
    // You may need to implement this based on your requirements
  };
    ...
    <div>
        ...
        <div className="flex justify-start ms-20 gap-4 mt-4">
          <button
            ...
            className={`btn ${listAllClicked ? 'bg-red-700 text-white shadow-lg shadow-red-950 scale-110 ' : 'bg-slate-600 border-black border'} text-black  px-4 py-2 transition duration-300 ease-in-out`}
          >
            Listed
          </button>
          <button
            onClick={handleUnlistAll}
            className={`btn ${unlistAllClicked ? 'bg-red-700 text-white shadow-lg shadow-red-950 scale-110' : 'bg-slate-600 border-black border '} text-black  px-4 py-2 transition duration-300 ease-in-out`}
          >
            Unlisted
          </button>
        </div>
        ...
        {list.map((video, index) => (
            ...
            >
              ...
              <div className="p-4">
                ...
                <button onClick={()=>handleshowcourses(video._id)} className={`${video.isShow ? 'text-yellow-500 hover:text-yellow-700':'text-green-500 hover:text-green-700'}btn hvr-shutter-in-horizontal justify-center border-y rounded-md border-black text-black bg-indigo-500 px-7 py-2 hover:bg-indigo-700 transition duration-300 ease-in-out`}>
            {video.isShow ? 
            <>
